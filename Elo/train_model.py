...
def modeling(X, Y, categorical, online):
    ...
    if online == 0:
        ...
        #X_train是划分之后的训练集。X_test是划分之后的测试集 Y_是对应的标签。 
        ...
        print('开始训练，注意设置好OPT-ROUNDS')
        dtrain = lgb.Dataset(X,
                             label=Y,
                             feature_name=list(X.columns),
                             categorical_feature=categorical)
        model = lgb.train(params, dtrain, num_boost_round=OPT_ROUNDS, valid_sets=[dtrain], valid_names=['train'],
                          verbose_eval=100)
